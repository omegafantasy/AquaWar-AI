...
public:
	...
	int friend_targets[12];					  //友方目标
	int enemy_targets[12];					  //敌方目标
	int friend_excepted_injury[12];			  //友方预期伤害
	int enemy_excepted_injury[12];			  //敌方预期伤害
	int friend_expected_injury[12];			  //友方预期伤害
	int enemy_expected_injury[12];			  //敌方预期伤害
	int num_friend_injury;								  //友方受伤鱼的数量
	int num_enemy_injury;								  //敌方受伤鱼的数量
	int friend_injury_id[24];		  //友方受伤鱼的位置编号
	int enemy_injury_id[24];		  //敌方受伤鱼的位置编号
	int friend_injury[24];			  //友方实际伤害
	int enemy_injury[24];			  //敌方实际伤害
	int friend_injury_timestamp[24];  //友方受伤时间戳
	int enemy_injury_timestamp[24];	  //敌方受伤时间戳
	bool friend_injury_traceable[24]; //友方受伤是否可追踪
	bool enemy_injury_traceable[24];  //敌方受伤是否可追踪
	skill_type type;
	//被动
	int num_friend_passives;							   //友方被动个数
	int num_enemy_passives;								   //敌方被动个数
	int friend_passives_id[24];		   //友方触发被动的鱼的id
	int enemy_passives_id[24];		   //敌方触发被动的鱼的id
	passive_type friend_types[24];	   //友方触发被动类型
	passive_type enemy_types[24];	   //敌方触发被动类型
	double friend_passive_value[24];   //友方减伤比/回血量
	double enemy_passive_value[24];	   //敌方减伤比/回血量
	int friend_passives_timestamp[24]; //友方触发被动时间戳
	int enemy_passives_timestamp[24];  //敌方触发被动时间戳
...
{
public:
	const Fish my_fish[4];
	const Fish enemy_fish[4];
	const int avatar_id;
